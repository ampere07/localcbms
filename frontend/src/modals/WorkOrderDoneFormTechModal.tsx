...
import { createWorkOrderItems, WorkOrderItem, deleteWorkOrderItems } from '../services/workOrderItemService';
...
interface WorkOrderDoneFormTechModalProps {
  isOpen: boolean;
  onClose: () => void;
...
interface WorkOrderDoneFormData {
...
const WorkOrderDoneFormTechModal: React.FC<WorkOrderDoneFormTechModalProps> = ({
  isOpen,
  onClose,
  onSave,
  workOrderData
  ...
  const [formData, setFormData] = useState<WorkOrderDoneFormData>({
      ...
      if (isOpen) {
        try {
          ...
          if (patterns && patterns.length > 0) {
            ...
            const existingUsername = workOrderData?.pppoe_username || workOrderData?.PPPoE_Username || '';
    ...
    if (workOrderData) {
      ...
      const getImageUrl = (fieldVariations: string[]): string | null => {
        for (const field of fieldVariations) {
          const value = workOrderData?.[field];
      ...
      const newImagePreviews = {
        signedContractImage: convertGoogleDriveUrl(workOrderData?.signed_contract_image_url || workOrderData?.Signed_Contract_Image_URL),
        setupImage: convertGoogleDriveUrl(workOrderData?.setup_image_url || workOrderData?.Setup_Image_URL),
        boxReadingImage: convertGoogleDriveUrl(workOrderData?.box_reading_image_url || workOrderData?.Box_Reading_Image_URL),
        routerReadingImage: convertGoogleDriveUrl(workOrderData?.router_reading_image_url || workOrderData?.Router_Reading_Image_URL),
        portLabelImage: convertGoogleDriveUrl(workOrderData?.port_label_image_url || workOrderData?.Port_Label_Image_URL),
        clientSignatureImage: convertGoogleDriveUrl(getImageUrl(clientSignatureVariations)),
        speedTestImage: convertGoogleDriveUrl(workOrderData?.speedtest_image_url || workOrderData?.Speedtest_Image_URL)
      };
  ...
  // Load existing work order items from database
  useEffect(() => {
    const fetchWorkOrderItems = async () => {
      if (isOpen && workOrderData) {
        ...
        if (workOrderId) {
          try {
            const response = await apiClient.get(`/job-order-items?job_order_id=${workOrderId}`);
  ...
  useEffect(() => {
    const fetchUsedPorts = async () => {
      ...
      try {
        // Fetch work orders to check for used ports
        ...
        if (response.data && response.data.success && Array.isArray(response.data.data)) {
          ...
          const currentId = workOrderData?.id || workOrderData?.WorkOrder_ID;
          ...
          response.data.data.forEach((jo: any) => {
            ...
            if (joLcpnap === formData.lcpnap) {
              ...
              const joId = jo.id || jo.WorkOrder_ID;

              // If port is used by ANOTHER work order, mark it as used
    ...
    fetchUsedPorts();
  }, [isOpen, formData.lcpnap, workOrderData]);
  ...
  useEffect(() => {
    if (workOrderData && isOpen) {
      ...
      const loadedOnsiteStatus = workOrderData.Onsite_Status || workOrderData.onsite_status || 'In Progress';
        ...
        try {
          ...
          if (applicationId) {
            ...
            if (appResponse.data.success && appResponse.data.application) {
              ...
              const newFormData = {
                dateInstalled: formatDateForInput(workOrderData.Date_Installed || workOrderData.date_installed) || getTodayDate(),
                usageType: getValue(workOrderData.Usage_Type || workOrderData.usage_type, 'usageType'),
                choosePlan: getValue(workOrderData.Desired_Plan || workOrderData.desired_plan || workOrderData.Choose_Plan || workOrderData.choose_plan || workOrderData.plan, 'choosePlan'),
                connectionType: getValue(workOrderData.Connection_Type || workOrderData.connection_type, 'connectionType'),
                routerModel: getValue(workOrderData.Router_Model || workOrderData.router_model, 'routerModel'),
                modemSN: getValue(workOrderData.Modem_SN || workOrderData.modem_sn, 'modemSN'),

                lcpnap: getValue(workOrderData.LCPNAP || workOrderData.lcpnap, 'lcpnap'),
                port: getValue(workOrderData.PORT || workOrderData.port, 'port'),
                vlan: getValue(workOrderData.VLAN || workOrderData.vlan, 'vlan'),
                region: getValue(appData.region || workOrderData.Region || workOrderData.region, 'region'),
                city: getValue(appData.city || workOrderData.City || workOrderData.city, 'city'),
                barangay: getValue(appData.barangay || workOrderData.Barangay || workOrderData.barangay, 'barangay'),
                onsiteStatus: loadedOnsiteStatus,
                onsiteRemarks: getValue(workOrderData.Onsite_Remarks || workOrderData.onsite_remarks, 'onsiteRemarks'),
                itemName1: getValue(workOrderData.Item_Name_1 || workOrderData.item_name_1, 'itemName1'),
                visit_by: getValue(workOrderData.Visit_By || workOrderData.visit_by, 'visit_by'),
                visit_with: getValue(workOrderData.Visit_With || workOrderData.visit_with, 'visit_with'),
                visit_with_other: getValue(workOrderData.Visit_With_Other || workOrderData.visit_with_other, 'visit_with_other'),
                statusRemarks: getValue(workOrderData.Status_Remarks || workOrderData.status_remarks, 'statusRemarks'),
                ip: getValue(workOrderData.IP || workOrderData.ip, 'ip'),
                addressCoordinates: getValue(workOrderData.Address_Coordinates || workOrderData.address_coordinates, 'addressCoordinates'),
              ...
              setImagePreviews({
                signedContractImage: safeConvert(workOrderData.signed_contract_image_url || workOrderData.Signed_Contract_Image_URL),
                setupImage: safeConvert(workOrderData.setup_image_url || workOrderData.Setup_Image_URL),
                boxReadingImage: safeConvert(workOrderData.box_reading_image_url || workOrderData.Box_Reading_Image_URL),
                routerReadingImage: safeConvert(workOrderData.router_reading_image_url || workOrderData.Router_Reading_Image_URL),
                portLabelImage: safeConvert(workOrderData.port_label_image_url || workOrderData.Port_Label_Image_URL),
                clientSignatureImage: safeConvert(workOrderData.client_signature_url || workOrderData.Client_Signature_URL),
                speedTestImage: safeConvert(workOrderData.speedtest_image_url || workOrderData.Speedtest_Image_URL)
              });
            }
          } else {
            loadDefaultFormData();
      ...
      const loadDefaultFormData = () => {
        const newFormData = {
          ...
          port: getValue(workOrderData.PORT || workOrderData.port, 'port'),
          vlan: getValue(workOrderData.VLAN || workOrderData.vlan, 'vlan'),
          region: getValue(workOrderData.Region || workOrderData.region, 'region'),
          city: getValue(workOrderData.City || workOrderData.city, 'city'),
          barangay: getValue(workOrderData.Barangay || workOrderData.barangay, 'barangay'),
          onsiteStatus: loadedOnsiteStatus,
          onsiteRemarks: getValue(workOrderData.Onsite_Remarks || workOrderData.onsite_remarks, 'onsiteRemarks'),
          itemName1: getValue(workOrderData.Item_Name_1 || workOrderData.item_name_1, 'itemName1'),
          visit_by: getValue(workOrderData.Visit_By || workOrderData.visit_by, 'visit_by'),
        ...
        setImagePreviews({
          signedContractImage: safeConvertDefault(workOrderData.signed_contract_image_url || workOrderData.Signed_Contract_Image_URL),
          setupImage: safeConvertDefault(workOrderData.setup_image_url || workOrderData.Setup_Image_URL),
          boxReadingImage: safeConvertDefault(workOrderData.box_reading_image_url || workOrderData.Box_Reading_Image_URL),
          routerReadingImage: safeConvertDefault(workOrderData.router_reading_image_url || workOrderData.Router_Reading_Image_URL),
          portLabelImage: safeConvertDefault(workOrderData.port_label_image_url || workOrderData.Port_Label_Image_URL),
          clientSignatureImage: safeConvertDefault(workOrderData.client_signature_url || workOrderData.Client_Signature_URL),
          speedTestImage: safeConvertDefault(workOrderData.speedtest_image_url || workOrderData.Speedtest_Image_URL)
        });
      };
  ...
  const handleInputChange = (field: keyof WorkOrderDoneFormData, value: string | File | null) => {
  ...
  const handleSave = async () => {
    ...
    console.log('[SAVE START] workOrderData:', workOrderData);
    console.log('[SAVE START] workOrderData.id:', workOrderData?.id);
    console.log('[SAVE START] workOrderData.WorkOrder_ID:', workOrderData?.WorkOrder_ID);
    ...
    if (!workOrderData?.id && !workOrderData?.WorkOrder_ID) {
      console.error('[SAVE ERROR] Cannot proceed - workOrderData is missing ID');
      console.error('[SAVE ERROR] workOrderData:', workOrderData);
      showMessageModal('Error', [
        { type: 'error', text: 'Cannot update work order: Missing ID' }
      ]);
      return;
    }

    console.log('[SAVE ID CHECK] work order has ID, proceeding...');
    ...
    try {
      ...
      if (!workOrderId) {
        ...
        console.error('[SAVE ERROR] Missing work order ID');
        console.error('[SAVE ERROR] workOrderData_keys:', Object.keys(workOrderData || {}));
        console.error('[SAVE ERROR] workOrderData_id:', workOrderData?.id);
        console.error('[SAVE ERROR] workOrderData_WorkOrder_ID:', workOrderData?.WorkOrder_ID);
        console.error('[SAVE ERROR] ========================================');

        saveMessages.push({
          type: 'error',
          text: 'Cannot update: Missing work order ID'
        });
      ...
      console.log('[SAVE ID CHECK] Work Order ID found:', workOrderId);
      console.log('[SAVE ID CHECK] typeof workOrderId:', typeof workOrderId);
      ...
      if (updatedFormData.onsiteStatus === 'Done') {
        workOrderUpdateData.connection_type = updatedFormData.connectionType;
        workOrderUpdateData.modem_router_sn = updatedFormData.modemSN;
        workOrderUpdateData.ip_address = updatedFormData.ip;
        workOrderUpdateData.onsite_remarks = updatedFormData.onsiteRemarks;
        workOrderUpdateData.address_coordinates = updatedFormData.addressCoordinates || '';
        workOrderUpdateData.onsite_status = 'Done';
        ...
        if (hasTechInput && techInputValue.trim()) {
          workOrderUpdateData.pppoe_username = techInputValue.trim();
          ...
          // Logic for RADIUS account creation moved to after updateWorkOrder
          // to ensure LCPNAP/Port data is saved first.
        }

        const firstName = (workOrderData?.First_Name || workOrderData?.first_name || '').trim();
        const middleInitial = (workOrderData?.Middle_Initial || workOrderData?.middle_initial || '').trim();
        const fullLastName = (workOrderData?.Last_Name || workOrderData?.last_name || '').trim();
        ...
        try {
          const uploadResponse = await apiClient.post<{
            ...
            folder_id?: string;
          }>(`/job-orders/${workOrderId}/upload-images`, imageFormData, {
            headers: {
          ...
          if (uploadResponse.data.success && uploadResponse.data.data) {
            ...
            if (imageUrls.signed_contract_image_url) {
              workOrderUpdateData.signed_contract_image_url = imageUrls.signed_contract_image_url;
            }
            if (imageUrls.setup_image_url) {
              workOrderUpdateData.setup_image_url = imageUrls.setup_image_url;
            }
            if (imageUrls.box_reading_image_url) {
              workOrderUpdateData.box_reading_image_url = imageUrls.box_reading_image_url;
            }
            if (imageUrls.router_reading_image_url) {
              workOrderUpdateData.router_reading_image_url = imageUrls.router_reading_image_url;
            }
            if (imageUrls.port_label_image_url) {
              workOrderUpdateData.port_label_image_url = imageUrls.port_label_image_url;
            }
            if (imageUrls.client_signature_image_url) {
              workOrderUpdateData.client_signature_url = imageUrls.client_signature_image_url;
            }
            if (imageUrls.speedtest_image_url) {
              workOrderUpdateData.speedtest_image_url = imageUrls.speedtest_image_url;
            }
          }
      ...
      if (updatedFormData.onsiteStatus === 'Failed' || updatedFormData.onsiteStatus === 'Reschedule') {
        workOrderUpdateData.onsite_remarks = updatedFormData.onsiteRemarks;
        workOrderUpdateData.status_remarks = updatedFormData.statusRemarks;
        workOrderUpdateData.onsite_status = updatedFormData.onsiteStatus;
      }

      if (updatedFormData.onsiteStatus === 'In Progress') {
        workOrderUpdateData.onsite_status = 'In Progress';

      }

      console.log('[API CALL] ========================================');
      console.log('[API CALL] ABOUT TO CALL: updateWorkOrder()');
      console.log('[API CALL] Method: PUT');
      console.log('[API CALL] Endpoint: /job-orders/' + workOrderId);
      console.log('[API CALL] Work Order ID:', workOrderId);
      console.log('[API CALL] typeof workOrderId:', typeof workOrderId);
      console.log('[API CALL] Update Data Keys:', Object.keys(workOrderUpdateData));
      console.log('[API CALL] Full Update Data:', JSON.stringify(workOrderUpdateData, null, 2));
      ...
      console.log('[API RESPONSE] updateWorkOrder completed');
      console.log('[API RESPONSE] Full response:', JSON.stringify(workOrderResponse, null, 2));
      console.log('[API RESPONSE] success:', workOrderResponse?.success);
      console.log('[API RESPONSE] message:', workOrderResponse?.message);
      console.log('[API RESPONSE] data:', workOrderResponse?.data);
      console.log('[API RESPONSE] ========================================');

      if (!workOrderResponse.success) {
        console.error('[SAVE ERROR] updateWorkOrder failed:', workOrderResponse);
        throw new Error(workOrderResponse.message || 'work order update failed');
      }

      console.log('[SAVE SUCCESS] work order updated successfully');
      ...
      if (updatedFormData.onsiteStatus === 'Done') {
        ...
        // Only proceed if NOT using manual tech input (manual input was already saved in updateWorkOrder)
        if (!(hasTechInput && techInputValue.trim())) {
          ...
          try {
            ...
            }>(`/job-orders/${workOrderId}/create-radius-account`);
            ...
            if (radiusResponse.data.success && radiusResponse.data.data) {
              ...
              // We need to update the work order AGAIN with the new credentials
              console.log('[SAVE RADIUS] Credentials generated, updating Work Order again...');
              ...
              try {
                await updateWorkOrder(workOrderId, credentialsUpdate);
                ...
                console.error('[SAVE RADIUS] Failed to save credentials to Work Order', updateError);
                saveMessages.push({
                  type: 'warning',
                  text: `RADIUS account created but failed to save to Work Order: ${(updateError as Error).message}`
                });
              }
      ...
      let applicationId = workOrderData.application_id || workOrderData.Application_ID || workOrderData.account_id;

      if (!applicationId) {
        try {
          const workOrderResponse = await apiClient.get<{ success: boolean; data: any }>(`/job-orders/${workOrderId}`);
          if (workOrderResponse.data.success && workOrderResponse.data.data) {
            applicationId = workOrderResponse.data.data.application_id;
          }
      ...
      if (applicationId) {
        try {
          const firstName = workOrderData?.First_Name || workOrderData?.first_name || '';
          const middleInitial = workOrderData?.Middle_Initial || workOrderData?.middle_initial || '';
          const lastName = workOrderData?.Last_Name || workOrderData?.last_name || '';
          const mobileNumber = workOrderData?.Mobile_Number || workOrderData?.mobile_number || '';
          const secondaryMobile = workOrderData?.Secondary_Mobile_Number || workOrderData?.secondary_mobile_number || '';
          const emailAddress = workOrderData?.Email_Address || workOrderData?.email_address || '';
          const installationAddress = workOrderData?.Installation_Address || workOrderData?.installation_address || '';
          const landmark = workOrderData?.Landmark || workOrderData?.landmark || '';
          const referredBy = workOrderData?.Referred_By || workOrderData?.referred_by || '';
          const promo = workOrderData?.Promo || workOrderData?.promo || '';
      ...
      if (updatedFormData.onsiteStatus === 'Done') {
        ...
        if (validItems.length > 0) {
          try {
            ...
            const numericWorkOrderId = parseInt((workOrderData.WorkOrder_ID || workOrderData.id || 0).toString());
            console.log('[SAVE ITEMS] Using Work Order ID:', numericWorkOrderId);

            if (!numericWorkOrderId || isNaN(numericWorkOrderId)) {
              throw new Error('Invalid Work Order ID for items saving');
            }

            // Get existing items from database
            const existingItemsResponse = await apiClient.get<{ success: boolean; data: any[] }>(`/job-order-items?job_order_id=${numericWorkOrderId}`);
            ...
            if (existingItemsResponse.data.success && Array.isArray(existingItemsResponse.data.data)) {
              ...
              for (const item of validItems) {
                ...
                if (existingItem) {
                  ...
                  itemsToCreate.push({ job_order_id: numericWorkOrderId, item_name: item.itemId, quantity: parseInt(item.quantity) });
                }
              }
              ...
              const itemsToCreate = validItems.map(item => ({
                job_order_id: numericWorkOrderId,
  ...
  const fullName = `${workOrderData?.First_Name || workOrderData?.first_name || ''} ${workOrderData?.Middle_Initial || workOrderData?.middle_initial || ''} ${workOrderData?.Last_Name || workOrderData?.last_name || ''}`.trim();
...
export default WorkOrderDoneFormTechModal;
