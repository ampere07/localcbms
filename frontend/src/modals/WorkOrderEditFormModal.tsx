...
import { X, Calendar, ChevronDown, Minus, Plus, Camera, MapPin, Loader2, Search } from 'lucide-react';
...
import { createWorkOrderItems, WorkOrderItem } from '../services/workOrderItemService';
...
interface WorkOrderEditFormModalProps {
  isOpen: boolean;
  onClose: () => void;
...
interface WorkOrderEditFormData {
  referredBy: string;
...
const WorkOrderEditFormModal: React.FC<WorkOrderEditFormModalProps> = ({
  isOpen,
  onClose,
  onSave,
  workOrderData
  ...
  const [formData, setFormData] = useState<WorkOrderEditFormData>({
    referredBy: '',
  ...
  useEffect(() => {
    const fetchUsedPorts = async () => {
      if (isOpen && formData.lcpnap) {
        try {
          const workOrderId = workOrderData?.id || workOrderData?.WorkOrder_ID;
          const response = await getUsedPorts(formData.lcpnap, workOrderId);
    ...
    fetchUsedPorts();
  }, [isOpen, workOrderData, formData.lcpnap]);
  ...
  useEffect(() => {
    if (workOrderData && isOpen) {
      const loadedStatus = workOrderData.Status || workOrderData.status || 'Confirmed';
          ...
          if (applicationId) {
            ...
            if (appResponse.data.success && appResponse.data.application) {
              ...
              setFormData(prev => ({
                ...
                dateInstalled: workOrderData.Date_Installed || workOrderData.date_installed || getTodayDate(),
                usageType: workOrderData.Usage_Type || workOrderData.usage_type || '',
                ...
                contactNumber: workOrderData.Mobile_Number || workOrderData.Contact_Number || workOrderData.mobile_number || workOrderData.contact_number || '',
                secondContactNumber: workOrderData.Secondary_Mobile_Number || workOrderData.second_contact_number || '',
                email: workOrderData.Email_Address || workOrderData.Applicant_Email_Address || workOrderData.email_address || workOrderData.email || '',
                address: workOrderData.Address || workOrderData.Installation_Address || workOrderData.address || workOrderData.installation_address || '',
                barangay: appData.barangay || workOrderData.Barangay || workOrderData.barangay || '',
                city: appData.city || workOrderData.City || workOrderData.city || '',
                region: appData.region || workOrderData.Region || workOrderData.region || '',
                addressCoordinates: workOrderData.Address_Coordinates || workOrderData.address_coordinates || '',
                choosePlan: workOrderData.Desired_Plan || workOrderData.desired_plan || workOrderData.Choose_Plan || workOrderData.choose_plan || workOrderData.plan || '',
                status: loadedStatus,
                connectionType: workOrderData.Connection_Type || workOrderData.connection_type || '',
                routerModel: workOrderData.Router_Model || workOrderData.router_model || '',
                modemSN: workOrderData.Modem_SN || workOrderData.modem_sn || '',

                lcpnap: workOrderData.LCPNAP || workOrderData.lcpnap || '',
                port: workOrderData.PORT || workOrderData.port || '',
                vlan: workOrderData.VLAN || workOrderData.vlan || '',
                username: workOrderData.Username || workOrderData.username || '',
                onsiteStatus: loadedOnsiteStatus,
                onsiteRemarks: workOrderData.Onsite_Remarks || workOrderData.onsite_remarks || '',
                contractLink: workOrderData.Contract_Link || workOrderData.contract_link || '',
                contractTemplate: (workOrderData.Contract_Template || workOrderData.contract_template || '1').toString(),
                assignedEmail: workOrderData.Assigned_Email || workOrderData.assigned_email || 'Office',
                itemName1: workOrderData.Item_Name_1 || workOrderData.item_name_1 || '',
                visit_by: workOrderData.Visit_By || workOrderData.visit_by || '',
                visit_with: workOrderData.Visit_With || workOrderData.visit_with || '',
                visit_with_other: workOrderData.Visit_With_Other || workOrderData.visit_with_other || '',
                statusRemarks: workOrderData.Status_Remarks || workOrderData.status_remarks || '',
                ip: workOrderData.IP || workOrderData.ip || '',
  ...
  const handleInputChange = (field: keyof WorkOrderEditFormData, value: string | File | null) => {
  ...
  const handleSave = async () => {
    ...
    if (!workOrderData?.id && !workOrderData?.WorkOrder_ID) {
      setModal({
        ...
        type: 'error',
        title: 'Error',
        message: 'Cannot update work order: Missing ID'
      });
      return;
    }
    ...
    try {
      ...
      if (updatedFormData.status === 'Confirmed') {
        if (updatedFormData.onsiteStatus === 'Done') {
          workOrderUpdateData.Date_Installed = updatedFormData.dateInstalled;
          workOrderUpdateData.Usage_Type = updatedFormData.usageType;
          ...
          workOrderUpdateData.Router_Model = updatedFormData.routerModel;
          workOrderUpdateData.Modem_SN = updatedFormData.modemSN;
          workOrderUpdateData.IP = updatedFormData.ip;
          workOrderUpdateData.LCPNAP = updatedFormData.lcpnap;
          workOrderUpdateData.PORT = updatedFormData.port;
          workOrderUpdateData.VLAN = updatedFormData.vlan;
          workOrderUpdateData.Onsite_Remarks = updatedFormData.onsiteRemarks;
          workOrderUpdateData.Item_Name_1 = updatedFormData.itemName1;
          workOrderUpdateData.Visit_By = updatedFormData.visit_by;
          workOrderUpdateData.Visit_With = updatedFormData.visit_with;
          workOrderUpdateData.Visit_With_Other = updatedFormData.visit_with_other;
        }
      ...
      setProgressSteps(prev => [...prev, 'Updating work order...']);
      ...
      if (updatedFormData.status === 'Confirmed' && updatedFormData.onsiteStatus === 'Done') {
        ...
        if (validItems.length > 0) {
          try {
            const existingItemsResponse = await apiClient.get<{ success: boolean; data: any[] }>(`/job-order-items?job_order_id=${workOrderId}`);
          ...
          const workOrderItems: WorkOrderItem[] = validItems.map(item => {
            return {
              job_order_id: parseInt(workOrderId.toString()),
          ...
          try {
            const itemsResponse = await createWorkOrderItems(workOrderItems);

            if (!itemsResponse.success) {
              throw new Error(itemsResponse.message || 'Failed to create work order items');
            }
            ...
            setModal({
              ...
              type: 'error',
              title: 'Error',
              message: `work order saved but items were not saved: ${errorMsg}`
            });
            setLoading(false);
      ...
      setModal({
        ...
        type: 'success',
        title: 'Success',
        message: 'Work Order and Application updated successfully!',
        onConfirm: () => {
          setErrors({});
...
export default WorkOrderEditFormModal;
