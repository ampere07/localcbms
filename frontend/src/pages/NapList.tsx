...
const NapList: React.FC = () => {
  ...
  const handleDelete = async (item: NAP, event: React.MouseEvent) => {
      ...
      <div className={`${isDarkMode ? 'bg-gray-900' : 'bg-white'
        ...
        <div className="flex flex-col h-full">
          ...
          <div className="flex-1 overflow-hidden flex flex-col">
            ...
            {/* Pagination Controls - Force visible if we have data */}
            {napItems.length > 0 && (
              <div className={`border-t p-4 flex items-center justify-between flex-shrink-0 ${isDarkMode ? 'bg-gray-900 border-gray-700' : 'bg-white border-gray-200'
                ...
                <div className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                  Showing <span className="font-medium">{(currentPage - 1) * itemsPerPage + 1}</span> to <span className="font-medium">{Math.min(currentPage * itemsPerPage, totalCount > 0 ? totalCount : napItems.length)}</span> of <span className="font-medium">{totalCount > 0 ? totalCount : napItems.length}</span> results
                </div>
