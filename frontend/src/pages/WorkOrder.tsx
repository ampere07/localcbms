...
import { FileText, Search, ChevronDown, ChevronRight, ListFilter, ArrowUp, ArrowDown, Menu, X, RefreshCw, Filter, Wrench } from 'lucide-react';
...
const WorkOrderPage: React.FC = () => {
  ...
  const [isAddWorkOrderOpen, setIsAddWorkOrderOpen] = useState<boolean>(false);
  ...
  const [activeFilters, setActiveFilters] = useState<any>(() => {
    const saved = localStorage.getItem('workOrderFilters');
    if (saved) {
      try {
  ...
  useEffect(() => {
    ...
    const refreshData = async () => {
      console.log('User idle for 15 minutes, auto-refreshing Work Order data...');
      try {
        const authData = localStorage.getItem('authData');
        let email: string | undefined;
        if (authData) {
    ...
    <div className={`${isDarkMode ? 'bg-gray-950' : 'bg-gray-50'
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            >
              <div className="flex items-center">
                ...
                <span className="capitalize text-base font-semibold">Assign Work Order</span>
              </div>
            </button>
            ...
            >
              <div className="flex items-center">
                ...
                <span className="capitalize text-base">All Work Orders</span>
              </div>
            ...
            <div className={`flex-1 ${displayMode === 'table' ? 'overflow-hidden' : 'overflow-y-auto'}`}>
              {isLoading ? (
                <div className={`px-4 py-12 text-center ${isDarkMode ? 'text-gray-400' : 'text-gray-600'
                  ...
                  <p className="mt-4">Loading work orders...</p>
                </div>
              ) : error ? (
      ...
      <JobOrderFunnelFilter
        ...
        onApplyFilters={(filters) => {
          ...
          localStorage.setItem('workOrderFilters', JSON.stringify(filters));
