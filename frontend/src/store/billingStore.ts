    ...
    refreshLatestData: async () => {
        ...
        try {
            ...
            if (result.data.length > 0) {
                ...
                console.log(`[BillingStore] Incremental refresh: ${brandNewRecords.length} new, ${result.data.length - brandNewRecords.length} updated`);
            } else {
                set({ lastFetchTimestamp: result.serverTime || now });
                console.log('[BillingStore] Incremental refresh: no changes found');
            }
